...
export class ECS {
    ...
    public addNewEntity(name: string, ...components: IComponent[]): Entity {
    ...
    public newEntity(name: string, ...components: IComponent[]): Entity {
        let e = new Entity(this, name);
        if (components.length > 0) {
            e.add(...components);
        }
    ...
    public getGroup(...componentTypes: string[]): Group {
